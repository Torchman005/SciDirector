...
**这是沙盒覆盖面上此前最大的一个洞**：三个 HTML 引擎（d3 / echarts / code_anim）渲染的是
LLM 生成的页面，而截图此前是在 **AI 服务进程内**直接 `sync_playwright()` 起 Chromium 的 ——
完全不经过 `SandboxRunner`，于是网络隔离、只读、seccomp 三块加固对它**一律无效**，
生成的 JS 可以在一个有网络的浏览器里自由外联。

现在逐帧截图搬进**沙盒子进程**（`html_capture.py`，经 runner 执行），三块加固自动生效。
`HtmlRenderer._capture` 只负责拼参数、把退出码映射成渲染失败 —— 边界越窄越不容易漂移。

**两个前置修复**：

1. **`RLIMIT_AS` 兜底下限 2GB → 128GB。** 逐项二分实测：Chromium **32GB 崩、64GB 正常**。
   原值让任何经过 runner 的浏览器启动瞬间 `SIGTRAP`，而报错里没有任何可读信息 ——
   看起来像"浏览器坏了"，实际是资源限制。
2. **`browser_ready()` 认 `SCID_CHROME` 覆盖**（仍验证文件存在，不是无条件放行）。
   Playwright 按 build 号装浏览器，包升级后期望的号会变，默认路径报
   `Executable doesn't exist`（本机就是这样）—— 不认覆盖就**无法验证**这条链路。
   生效后 `/healthz` 的 `d3/echarts/code_anim` 从 missing 变为 **ok**。

**证据**（`tests/test_html_sandbox.py`，8 项，全部实跑无跳过）：

| 断言 | 说明 |
| --- | --- |
| 沙盒里页面 JS 对公网 `fetch` 失败 | 核心安全断言：生成的 JS 连不出去 |
| **不隔离**时本机回环 HTTP 连得上 | 确定性反向对照（用公网做对照会时而跳过，而"时跳过的对照"等于没有对照） |
| 隔离下**连回环也不通** | 证明网络命名空间真的建起来了 |
| 整条 HTML 路径在沙盒下出片 | 截图 → 编码 → probe，校验时长与分辨率 |
| 缺 `window.__seek` → 退出码 3 | 内容问题与环境问题必须能分开处置（前者回灌给编码智能体，后者重试/转人工） |

**变异验证**：把截图改回"进程内直接调用"（即本次修复要防住的回退），用例立刻变红。

**没验证的**：无法用 mock LLM 端到端跑通 HTML 镜头 —— mock 生成的 HTML 缺
`window.__seek`，在 `code` 节点的静态检查就熔断转人工（实测 `job-e2d2cacf057b04c1` 的
DATA 镜头 event 11~16 全是契约检查失败），**根本没走到渲染**。
因此这条链路的证据来自直接驱动 `HtmlRenderer.render` 的用例，而不是整条任务。
真实任务里也能确认这一点：日志中**没有任何**截图或浏览器失败。


### 模型服务商（v0.6.2）

**支持三家 + mock**，全部走 OpenAI 兼容协议：`openai` / `deepseek`（DeepSeek）/
`bailian`（阿里云百炼，兼容端点 `/compatible-mode/v1`）/ `mock`。
文本与视觉可分别选（`SCID_VLM_PROVIDER`，留空跟随文本）——
因为 **DeepSeek 没有视觉模型**，"DeepSeek 写代码 + 百炼审画面"是推荐组合。

**没配视觉时不会伪造审查通过**：真文本 + 视觉不可用 ⇒ 抛错并由 Critic 降级转人工。
这条是刻意的（mock 审查会返回伪造的通过结论，让未审查的画面进成片且无报错）。

**验证**

| 层次 | 内容 |
| --- | --- |
| 解析（11 项） | 各家默认/base_url/密钥不串、显式模型覆盖、通用覆盖优先、非法 provider **启动即失败** |
| 接线（本地假服务端） | 断言服务端真的收到 `Authorization`、`model`、以及视觉请求里的 `image_url` 分片 |
| **真实端点** | 本机实测 `api.deepseek.com` 与 `dashscope.aliyuncs.com` 可达：无效密钥得到 **401** 且**只发 1 次请求**；`api.openai.com` 不通，用例显式记录"验不了" |
| 安全规则 | 真文本 + 无视觉 ⇒ 拒绝调用（不退回 mock），带变异验证 |

**未验证**：真实补全（需要有效密钥）——因此"模型能否按要求产出 JSON"
以及"百炼的 qwen-vl-max 对抽帧的审查质量"都**没有**实测数据。
...
- **状态对账**：✅ 定期比对 + 自动修一类（见下）
- **沙盒加固**：✅ 网络隔离（`unshare -r -n`）+ 只读根（`unshare -r -m`）+ seccomp 过滤（`libseccomp`，**拒绝名单**而非白名单）+ **HTML 引擎的浏览器也进了沙盒**；**真正的容器/cgroup 层未做**
...
- ✅ 沙盒在无网络条件下仍能完成渲染（证明没有隐式外联）
      （已验：外联与 DNS 被挡住、**同一台机器不隔离时能连出去**这个反向对照成立、
       ...
       三层全开时真实任务照常出片；
       **HTML 引擎的浏览器也已进沙盒**，且用"页面 JS 连不出去"直接验证过。
       **未做**：真正的容器/cgroup 层；manim/LaTeX 本机不可用因此 manim 路径未验证；
       seccomp 只在 stock/ffmpeg 与 HTML 路径上验证过）
