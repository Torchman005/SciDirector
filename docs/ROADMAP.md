...
**仍待办**：把 TTS 接进流水线 —— 目前适配层与配置已就绪（缺省关闭，不影响既有行为），
但**图节点还没有调用它**，因此镜头目前仍不会产出 `audio_path`。下一步是：
在渲染节点里对每个镜头合成配音、写进 `RenderArtifact.audio_path` 并落 sidecar，
然后用真实 Edge TTS 跑一遍端到端（Go 侧消费音频的那一半已完成并验证）。
