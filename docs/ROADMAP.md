...
**仍待办（一小块）**：句级时间戳 sidecar 已经**写出来并落在音频旁边**，
但 **Go 侧还没有消费它** —— 当前字幕按「镜头真实音频时长」对齐（已上图验证），
镜头内部仍是按文本比例分配。要再进一步（按真实句子起止对齐），
需要在 Go 的 `PlanCuesWithNarration` 里读 sidecar 并把句级时间换算到成片时间轴。
这属于增量优化，不是缺口：现在的行为已经比「按画面窗口铺满」准确得多。
