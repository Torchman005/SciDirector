...
- **容器级加固（`--network=none --read-only`）仍未做。** 本机 `unshare -n` 不可用
  （非 root 无权建网络命名空间），命名空间路径是本机可用的等价手段；
  生产若跑容器，容器层仍应作为**外层**防御（纵深防御，不是二选一）。
