...
>
> 版本：v0.4.3 · 阶段：阶段五（生产加固）· 最后更新：见文末「迭代日志」
...
| `minio/minio` 已不是可以安心依赖的对象存储 | MinIO 开源版**已归档停更**：`dl.min.io` 返回 410，GitHub 上最后一个 release 没有任何可下载产物，官方声明不再提供安全更新。compose 里钉的 `minio/minio:latest` 可用性需要重新确认；产线应选仍在维护的 S3 兼容实现（RustFS / SeaweedFS / Garage）。详见 `docs/ROADMAP.md` |
